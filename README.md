...
![Asynchronous FIFO](https://img.shields.io/badge/Design-Asynchronous%20FIFO-orange)
![8-bit](https://img.shields.io/badge/Data%20Width-8--bit-green)
![8-depth](https://img.shields.io/badge/Depth-8-purple)
![Icarus Verilog](https://img.shields.io/badge/Simulation-Icarus%20Verilog-red)
![GTKWave](https://img.shields.io/badge/Waveform-GTKWave-yellow)
...
This project implements an **8-bit × 8-depth Asynchronous FIFO (First-In First-Out)** using **Verilog HDL**.

The FIFO transfers data between two independent clock domains:

- **Write Clock Domain** — `wr_clk`
- **Read Clock Domain** — `rd_clk`

The design uses **binary pointers, Gray-code pointers, and 2-flop synchronizers** to safely transfer pointer information between the two clock domains.
...
- Implement an 8 × 8 FIFO memory.
- Generate binary write and read pointers.
- Convert binary pointers into Gray code.
- Synchronize pointer information across clock domains.
- Implement `full` and `empty` detection.
- Prevent writing when the FIFO is full.
- Prevent reading when the FIFO is empty.
- Develop a Verilog testbench.
- Verify FIFO functionality using simulation waveforms.
...
| CDC Technique | Gray-code Pointer Synchronization |
| Synchronization | 2-Flop Synchronizer |
...
| Block | Function |
|-------|----------|
| Write Controller | Controls write pointer, address and full flag |
| FIFO Memory | Stores 8-bit data |
| Read Controller | Controls read pointer, address and empty flag |
| Read-to-Write Synchronizer | Transfers read pointer information to write domain |
| Write-to-Read Synchronizer | Transfers write pointer information to read domain |
| Top Module | Connects all FIFO components |
...
1. Checks whether the FIFO is full.
2. Generates the write address.
3. Stores input data into the FIFO memory.
4. Increments the binary write pointer.
5. Converts the binary write pointer into Gray code.
6. Updates the full status.
...
1. Checks whether the FIFO is empty.
2. Generates the read address.
3. Reads data from the FIFO memory.
4. Increments the binary read pointer.
5. Converts the binary read pointer into Gray code.
6. Updates the empty status.
...
Since 2³ = 8, three address bits are required.
...
The additional MSB provides wrap-around or cycle information, which is required for correct full and empty detection.
...
In normal binary counting, multiple bits can change during a transition. For example:
...
In Gray code, only one bit changes between consecutive values. Therefore, Gray-coded pointers are suitable for transferring pointer information between asynchronous clock domains.
...
The first flip-flop samples the asynchronous signal. The second flip-flop provides the synchronized signal to the destination-domain logic. This reduces the probability of metastability propagating into the destination clock domain.
...
The verified GTKWave waveform is included in this repository.

> **⚠️ Simulation Note:** At the beginning of simulation, `data_out` may appear as `xx`. This is expected because `data_out` is a register and has not yet received valid read data. After a valid read operation, the expected data sequence appears (`11 → 22 → 33 → 44 → 55 → 66 → 77 → 88`). Therefore, the initial `xx` value does not affect the verified FIFO data sequence.
...
```
wr_clk, rd_clk, wr_en, rd_en, data_in, data_out, full, empty,
wr_bin, rd_bin, wr_gray, rd_gray, wr_gray_sync, rd_gray_sync
```
...
Through this project, practical understanding was gained of:
...
The project provides practical exposure to RTL architecture, FIFO design, clock-domain crossing, Gray-code synchronization, digital design, simulation, and waveform-based verification.
